...
/*   Updated: 2017/09/18 17:00:38 by czalewsk         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "ft_ls.h"

static void			ls_format_line_n(char *line, t_ls_ext *info,
		t_ls_info *file)
{
	t_ls_ln		*ln;
	char		*tmp;
	...
	*line = '\0';
	...
	(tmp = ft_itoa(ln->link)) && ft_cpy_until_r(line, tmp, ' ', info->link);
	free(tmp);
	ft_cpy_until_r(line, ln->owner, ' ', info->user + 1);
	ft_strcat(line, "  ");
	ft_cpy_until(line, ln->group, ' ', info->group + 1);
	(tmp = ft_itoa(ln->size)) && ft_cpy_until_r(line, tmp, ' ', info->size + 1);
	free(tmp);
	ft_strcat(line, " ") && ft_strncat(line, ln->time, 14);
